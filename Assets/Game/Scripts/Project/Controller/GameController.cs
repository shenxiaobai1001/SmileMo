...
public class GameController : MonoBehaviour
{
    ...
    public void OnSetGameLevel(int level)
    {
        ...
        PlayerController.Instance.OnSetStickCheck(gameLevel != 1&& gameLevel != 2 && gameLevel != 3 && gameLevel != 5);
    ...
    public void PlayerRestToSavePos(object msg)
    {
        ...
        OnSetGameLevel(savePos[saveIndex - 1].GetComponent<SaveItem>().level);
    }

    public void OnMoveToNextSavePos()
    {
        if (saveIndex <= 1)
            saveIndex++;
        else if (saveIndex >= savePos.Count)
            saveIndex = 1;
        else
        {
            saveIndex++;
    ...
    public void OnMoveToOldSavePos()
    {
        if (saveIndex <= 1)
            saveIndex = savePos.Count-1;
        else
        {
            saveIndex--;
        }
