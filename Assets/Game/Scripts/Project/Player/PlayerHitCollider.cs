...
public class PlayerHitCollider : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (collision.gameObject.CompareTag("Monster") && !controller.isCheckVec)
        {
            if (GameController.Instance.gameLevel == 7)
            {
                LevelSevenController.Instance.OnOutPlayerSevenFail();
            }
            Sound.PlaySound("Sound/PlayerHit");
            if (controller) controller.isCheckVec = true;
            if (controller) controller.HandleVecHitCollision();
        ...
        else if (collision.gameObject.name.Contains("Banana"))
        {
            Sound.PlaySound("Sound/PlayerHit");
            if (controller) controller.isCheckVec = true;
            if (controller) controller.HandleVecHitCollision();
